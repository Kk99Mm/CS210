...
def collaborative_filtering_recommendations(customer_id):
    ...
    if connection is not None:
        ...
        # Prioritize the products based on how many similar customers have purchased them
        product_priority = {}
        for product in recommended_products:
            product_priority[product] = sum(df[df["product_id"] == product]["customer_id"].isin(similar_customers))

        # Sort products by priority (most recommended by similar customers first)
        sorted_recommendations = sorted(product_priority.items(), key=lambda x: x[1], reverse=True)
        
        # Return the product IDs sorted by relevance
...
def rule_based_recommendations(customer_id):
    ...
    if connection is not None:
        ...
        # Sort recommendations by sales (this can be changed based on other metrics)
